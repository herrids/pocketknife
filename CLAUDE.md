...
A manifest function is **one of two closed kinds**: a *prompt function* (`prompt`: an LLM template with `{{param}}` placeholders, capabilities exactly `{"model": true}`) or a *wasm function* (`entry`: a pre-built module). `funcrun/` executes both behind `POST /apps/{app}/functions/{name}` (served by `api/functions.go`; "functions" is a reserved entity name so no manifest can shadow the route). The placeholder syntax is defined once in `schema/prompt.go` and mirrored in the agent's TS seams. `sandbox/` is the **real** security boundary for wasm (the manifest only *declares* capabilities): adversarial WebAssembly under wazero with no filesystem, no env, no raw network — the only way out is the fixed, capability-checked host ABI (`host.go`), with per-invocation resource limits; denials carry no payload so a function can't use responses as an oracle. `broker/` is the **only** path to a model provider — selected once at boot by `brokerFromEnv` (`ANTHROPIC_API_KEY` → Anthropic Messages API, optional `POCKETKNIFE_MODEL` override; else `POCKETKNIFE_MODEL_ENDPOINT`/`_TOKEN` generic caller; else unconfigured → 503 `model_not_configured`) — the credential is held unexported and never reaches a function or the browser. `consent/` derives the union of capabilities an app's functions request (pure function of the manifest), for a future shell to render. **Hand-synced mirrors** (a manifest/client change must land on both sides): `manifest.schema.json` ↔ `agent/schema/manifest.schema.json`, `client/generate.go` ↔ `agent/src/seams/generate-client.ts`, `validate/semantic.go` ↔ `agent/src/seams/semantic.ts`. Note: `migrate.Diff` covers entities only, but `migrate.Apply` still promotes + re-registers a version-bumped manifest whose entity diff is empty (e.g. one that only adds a function); adding a function requires a version bump — a same-version redeploy never re-reads the manifest.
